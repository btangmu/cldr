...
public class InterestLocales {
    ...
    public Response setIntLocs(IntLocsRequest request) {
        ...
        // intlocs may be null or empty for deletion; not an error;
        // we might consider a different protocol for deletion
    ...
    private Response reallySetIntLocs(CookieSession session, String intlocs) {
         ...
         */
        String msg = CookieSession.sm.reg.setLocales(session, session.user.id, session.user.email, intlocs, true);
        r.put("status", msg);
